...
SOURCE_DIR = "Forex_EAs" 

# Tüm .ex4 ve .ex5 dosyalarının toplanacağı yeni hedef klasör
DEST_DIR = "All_Compiled_EAs"
...
def collect_compiled_files():
    ...
    for root, dirs, files in os.walk(SOURCE_DIR):
        for file in files:
            # Sadece .ex4 ve .ex5 uzantılı dosyaları yakala
            if file.lower().endswith(('.ex4', '.ex5')):
                ...
                try:
                    # Dosyayı hedef klasöre kopyala (shutil.copy2 meta verilerini de korur)
    ...
    print(f"\nİşlem tamamlandı! Toplam {copied_count} adet .ex4 ve .ex5 dosyası '{DEST_DIR}' klasöründe toplandı.")
    print(f"Dosyaların konumu: {os.path.abspath(DEST_DIR)}")

if __name__ == "__main__":
    collect_compiled_files()